=== Flying Analytics: Self-Host Google Analytics v4 with Speed Optimization ===

Contributors: gijo
Donate link: https://www.buymeacoffee.com/gijovarghese
Tags: google analytics, minimal analytics 
Requires at least: 4.5
Tested up to: 6.9
Requires PHP: 5.6
Stable tag: 2.0.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Self-host Google Analytics v4 or use Minimal Analytics, a lightweight gtag.js alternative, to improve site speed and enhance privacy.

== Description ==

Host Google Analytics locally using any one of these:

- Gtag.js (90KB)
- Minimal Analytics.js (1.4KB)

Optionally, disable Analytics when admin is logged in.

## Support
- [Official Support Forum](https://wordpress.org/support/plugin/flying-analytics/)
- [Facebook Group](https://www.facebook.com/groups/wpspeedmatters)

## Our premium products
- [FlyingPress](https://flyingpress.com)
- [FlyingCDN](https://flyingcdn.com)

## Our free plugins
- [Flying Pages](https://wordpress.org/plugins/flying-pages/)
- [Flying Scripts](https://wordpress.org/plugins/flying-scripts/)
- [Flying Images](https://wordpress.org/plugins/nazy-load/)
- [Flying Analytics](https://wordpress.org/plugins/flying-analytics/)
- [Flying Fonts](https://wordpress.org/plugins/flying-fonts/)

== Installation ==

1. Visit 'Plugins > Add New'
1. Search for 'Flying Analytics'
1. Activate Flying Analytics for WordPress from your Plugins page.
1. Visit 'Settings -> Flying Analytics' to configure

== Screenshots ==
1. Flying Analytics Settings

== Changelog ==

= 2.0.0 =
- New: Google Analytics v4 support

= 1.1.3 =
- Security updates

= 1.1.2 =
- [BUGFIX] 404 error on js files

= 1.1.1 =
- [BUGFIX] Warning for constant _FILE_

= 1.1.0 =
- [NEW] Disable for logged in admins (credits: [Gulshan Kumar](https://gulshankumar.net/))

= 1.0.0 =
- Initial release
